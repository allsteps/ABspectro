...
class SpectrometerApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.adapt_min_button = tk.Button(self.control_frame, text="Adapt Min Wavelength", command=self.adapt_min_wavelength)
        self.adapt_min_button.pack(pady=5)

        self.adapt_max_button = tk.Button(self.control_frame, text="Adapt Max Wavelength", command=self.adapt_max_wavelength)
        self.adapt_max_button.pack(pady=5)
        
        self.save_ref_button = tk.Button(self.control_frame, text="Save reference data", command=self.save_reference_spectrum)
        ...
        self.save_trans_button = tk.Button(self.control_frame, text="Save transmittance data", command=self.save_transmittance_spectrum_as_text)
        ...
        self.save_3d_button = tk.Button(self.control_frame, text="Save 3D data", command=self.save_final_3d_data)
    ...
    def disable_all_buttons(self):
        ...
        self.adapt_min_button.config(state=tk.DISABLED)
        self.adapt_max_button.config(state=tk.DISABLED)
    ...
    def plot_reference_and_realtime_measurement(self):
        ...
        self.canvas.draw()
        self.enable_buttons([self.adapt_min_button])
    ...
    def adapt_min_wavelength(self):
        # Adapt the minimum wavelength range based on the light intensity
        self.take_reference_measurement()
        if len(reference_wavelengths) == 0 or len(reference_intensities) == 0:
            print("No reference data to adapt.")
            return

        # Create a DataFrame from the reference data
        df = pd.DataFrame({'wavelength': reference_wavelengths, 'intensity': reference_intensities})

        # Debug: Print the initial DataFrame and its types
        ...
        threshold = 5000  # Adjust this threshold value
        df_filtered = df[df['intensity'] > threshold]

        # Debug: Print the filtered DataFrame
        print("Filtered DataFrame:")
        print(df_filtered)
        print("Filtered DataFrame types:")
        ...
        # Update the wavelength min value based on the first occurrence above the threshold
        self.wavelength_min = df_filtered['wavelength'].iloc[0]

        # Additional filtering to exclude the default base values of the spectrometer
        if self.spectrometer.model == 'USB2000PLUS' and self.wavelength_min < 200:
            self.wavelength_min = df_filtered[df_filtered['wavelength'] > 200]['wavelength'].iloc[0]
        elif self.spectrometer.model == 'SR2' and self.wavelength_min < 649:
            self.wavelength_min = df_filtered[df_filtered['wavelength'] > 649]['wavelength'].iloc[0]

        print(f"Updated minimum wavelength: {self.wavelength_min}")

        # Redraw the plot with the new wavelength range
        self.ax.set_xlim(self.wavelength_min, self.wavelength_max)
        self.ax.relim()
        self.ax.autoscale_view()
        self.canvas.draw()
        self.enable_buttons([self.adapt_max_button])
        self.disable_buttons([self.adapt_min_button])


    def adapt_max_wavelength(self):
        # Adapt the maximum wavelength range based on the light intensity
        self.take_reference_measurement()
        if len(reference_wavelengths) == 0 or len(reference_intensities) == 0:
            print("No reference data to adapt.")
            return
        ...
        # Update the wavelength max value based on the last occurrence above the threshold
        self.wavelength_max = df_filtered['wavelength'].iloc[-1]

        # Additional filtering to exclude the default base values of the spectrometer
        if self.spectrometer.model == 'USB2000PLUS' and self.wavelength_max > 850:
            self.wavelength_max = df_filtered[df_filtered['wavelength'] < 850]['wavelength'].iloc[-1]
        elif self.spectrometer.model == 'SR2' and self.wavelength_max > 1300:
            self.wavelength_max = df_filtered[df_filtered['wavelength'] < 1300]['wavelength'].iloc[-1]

        print(f"Updated maximum wavelength: {self.wavelength_max}")
        ...
        self.disable_buttons([self.adapt_max_button])
    ...
    def plot_3d_graph(self):
        ...
        filtered_wavelengths = [w for w in reference_wavelengths if self.wavelength_min <= w <= self.wavelength_max]
        
        for i, angle in enumerate(all_angles):
            filtered_transmittances = [t for w, t in zip(reference_wavelengths, all_transmittances[i]) if self.wavelength_min <= w <= self.wavelength_max]
            self.ax_3d.plot(filtered_wavelengths, [angle]*len(filtered_wavelengths), filtered_transmittances)
    ...
    def save_final_3d_data(self):
        ...
        if final_3d_file_path:
            try:
                ...
                print(f"Final 3D data saved successfully at {final_3d_file_path}.")
            except Exception as e:
